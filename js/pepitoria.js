...
let fotoDos = 0;
let numFotosDos = 10
let anchoImagenDos = 100 / numFotosDos

const nextDos = document.querySelector(`.NextDos`)
const prevDos = document.querySelector(`.PrevDos`)
const slideDosWrapper = document.querySelector(`.SlideDos-wrapper`)
const slideDosDiv = document.querySelectorAll(`.SlideDos-div`)

let mostrarImagenDos = () => {
    slideDosWrapper.style.transform = `translateX(-${anchoImagenDos * fotoDos}%)`

    slideDosDiv.forEach((_, i) => {
        slideDosDiv[i].classList.remove(`isActive`)
    })
    slideDosDiv[fotoDos].classList.add(`isActive`)
}

nextDos.addEventListener(`click`, () => {
    fotoDos++
    if (fotoDos >= 10) {
        fotoDos = 0
    }
...
prevDos.addEventListener(`click`, () => {
    fotoDos--
    if (fotoDos <= -1) {
        fotoDos = 9
    }
